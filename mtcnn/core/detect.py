...
class MtcnnDetector(object):
    ...
    def create_mtcnn_net(self):
        ...
        if len(self.args.pnet_file) > 0:
            ...
            if self.args.use_cuda:
                ...
                pnet = torch.nn.DataParallel(pnet, device_ids=self.args.gpu_ids).cuda()
            else:
                pnet.load_state_dict(torch.load(self.args.pnet_file,\
        ...
        if len(self.args.rnet_file) > 0:
            ...
            if self.args.use_cuda:
                ...
                rnet = torch.nn.DataParallel(rnet, device_ids=self.args.gpu_ids).cuda()
            else:
                rnet.load_state_dict(torch.load(self.args.rnet_file,\
                                                map_location=lambda storage, loc: storage))
            rnet.eval()
        ...
        if len(self.args.onet_file) > 0:
            ...
            if self.args.use_cuda:
                ...
                onet = torch.nn.DataParallel(onet, device_ids=self.args.gpu_ids).cuda()
            else:
                onet.load_state_dict(torch.load(self.args.onet_file, \
                                                map_location=lambda storage, loc: storage))
            onet.eval()
    ...
    def detect_pnet(self, im):
        ...
        while min(current_height, current_width) > net_size:
            ...
            boxes = self.generate_bbox(cls_map_np[ 0, :, :], reg_np, current_scale, self.args.prob_thres[0])
    ...
    def detect_face(self, img, verbose = True):
